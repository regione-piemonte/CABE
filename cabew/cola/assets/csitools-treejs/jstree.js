...
(function (factory) {
	...
	var instance_counter = 0,
		...
		_d = document, _node = _d.createElement('LI'), _temp0, _temp1, _temp2, _temp3;

	_node.setAttribute('role', 'treeitem');
	_temp0 = _d.createElement('DIV');
	_temp0.className = 'row treenode-cont';



	_temp1 = _d.createElement('SPAN');
	_temp1.className = 'dnd col-md-1';
	_temp2 = _d.createElement('I');
	_temp2.className = 'fa fa-bars';
	_temp1.appendChild(_temp2);
	_temp0.appendChild(_temp1);
	_temp1 = _d.createElement('SPAN');
	_temp1.className = 'content col-md-10';
	_temp2 = _d.createElement('SPAN');
	_temp2.className = 'icon jstree-ocl';
	_temp3 = _d.createElement('I');
	_temp3.className = 'fa fa-plus';
	_temp2.appendChild(_temp3);
	_temp1.appendChild(_temp2);
	_temp2 = _d.createElement('SPAN');
	_temp2.className = 'label';
	_temp3 = _d.createElement('A');
	_temp3.className = 'jstree-anchor';
	_temp3.setAttribute('href', '#');
	_temp2.appendChild(_temp3);
	_temp1.appendChild(_temp2);
	_temp0.appendChild(_temp1);
	...
	_temp1 = _d.createElement('SPAN');
	_temp1.className = 'col-md-1';
	_temp2 = _d.createElement('STRONG');
	_temp2.className = 'controls';
	_temp3 = _d.createElement('I');
	_temp3.className = 'cerchietto';
	_temp2.appendChild(_temp3);
	_temp1.appendChild(_temp2);
	_temp0.appendChild(_temp1);

	_temp1 = _d.createElement('SPAN');
	_temp1.className = 'type col-md-1';
	_temp2 = _d.createElement('I');
	_temp2.className = 'jstree-icon jstree-themeicon';
	_temp1.appendChild(_temp2);
	_temp0.appendChild(_temp1);

	_node.appendChild(_temp0);
	_temp0 = _temp1 = _temp2 = _temp3 = null;
	...
	$.jstree.core.prototype = {
		...
		init: function (el, options) {
			...
			this._data.core.original_container_html
				...
				.remove();
			this.element.html("<" + "ul class='jstree-container-ul jstree-children'><" + "li class='jstree-initial-node jstree-loading jstree-leaf jstree-last'><i class='jstree-icon jstree-ocl'></i><" + "a class='jstree-anchor' href='#'><i class='jstree-icon jstree-themeicon-hidden'></i></a></li></ul>");
		...
		_parse_model_from_html: function (d, p, ps) {
			...
			data.state.selected = d.children('.treenode-cont').hasClass('jstree-clicked');
		...
		redraw_node: function (node, deep, is_callback) {
			...
			$(node).find('.content').css('padding-left', (this.get_node(obj).parents.length * 15) + 'px');
			...
			if (c.length) {
				node.childNodes[0].className += c;
			}
			if ((obj.icon && obj.icon !== true) || obj.icon === false) {
				if (obj.icon === false) {
					node.childNodes[0].childNodes[3].childNodes[0].className += ' jstree-themeicon-hidden';
				}
				else if (obj.icon.indexOf('/') === -1 && obj.icon.indexOf('.') === -1) {
					node.childNodes[0].childNodes[3].childNodes[0].className += ' ' + obj.icon + ' jstree-themeicon-custom';
				}
				else {
					node.childNodes[0].childNodes[3].childNodes[0].style.backgroundImage = 'url(' + obj.icon + ')';
					node.childNodes[0].childNodes[3].childNodes[0].style.backgroundPosition = 'center center';
					node.childNodes[0].childNodes[3].childNodes[0].style.backgroundSize = 'auto';
					node.childNodes[0].childNodes[3].childNodes[0].className += ' jstree-themeicon-custom';
				}
			}

			if (this.settings.core.force_text) {
				node.childNodes[0].childNodes[1].childNodes[1].childNodes[0].appendChild(d.createTextNode(obj.text));
			}
			else {
				node.childNodes[0].childNodes[1].childNodes[1].childNodes[0].innerHTML += obj.text;
			}
			...
			if (deep && obj.children.length && obj.state.opened && obj.state.loaded) {
				...
				for (i = 0, j = obj.children.length; i < j; i++) {
					var tmpnode = this.redraw_node(obj.children[i], deep, true);
					//$(tmpnode).find('.content').css('padding-left', (this.get_node(tmpnode).parents.length * 15) + 'px');
					k.appendChild(tmpnode);
				}
				node.appendChild(k);
			}
			if (old) {
		...
		select_node: function (obj, supress_event, prevent_open, e) {
			...
			if (!obj.state.selected) {
				...
				if (dom && dom.length) {
					dom.children('.treenode-cont').addClass('jstree-clicked');
				}
				/**
		...
		deselect_node: function (obj, supress_event, e) {
			...
			if (obj.state.selected) {
				...
				if (dom.length) {
					dom.children('.treenode-cont').removeClass('jstree-clicked');
				}
				/**
		...
		refresh: function (skip_loading, forget_state) {
			...
			if (!skip_loading) {
				this.element.html("<" + "ul class='" + c + "'><" + "li class='jstree-initial-node jstree-loading jstree-leaf jstree-last'><i class='jstree-icon jstree-ocl'></i><" + "a class='jstree-anchor' href='#'><i class='jstree-icon jstree-themeicon-hidden'></i></a></li></ul>");
				jQuery('#csi-overlay').css('display', 'block');
		 ...
		 * CSI AF 20211008 aggiunta funzione mancante sulla vecchia versione
		 */
				...
				this.element
							...
							for (k = 0, l = chd.length; k < l; k++) {
								...
								while (p && p.id !== '#') {
									...
									if (c === j) {
										...
										if (tmp && tmp.length) {
											tmp.children('.treenode-cont').addClass(t ? 'jstree-clicked' : 'jstree-checked');
										}
									}
									else {
										break;
					...
					.on(this.settings.checkbox.tie_selection ? 'select_node.jstree' : 'check_node.jstree', $.proxy(function (e, data) {
						...
						if (s.indexOf('down') !== -1 && dom.length) {
							dom.find('.treenode-cont').addClass(t ? 'jstree-clicked' : 'jstree-checked');
						}
					}, this))
					...
					.on(this.settings.checkbox.tie_selection ? 'deselect_node.jstree' : 'uncheck_node.jstree', $.proxy(function (e, data) {
						...
						if (s.indexOf('up') !== -1) {
							for (i = 0, j = obj.parents.length; i < j; i++) {
								...
								if (tmp && tmp.length) {
									tmp.children('.treenode-cont').removeClass(t ? 'jstree-clicked' : 'jstree-checked');
								}
							}
						}
						tmp = [];
						...
						// apply down (process .children separately?)
						if (s.indexOf('down') !== -1 && dom.length) {
							dom.find('.treenode-cont').removeClass(t ? 'jstree-clicked' : 'jstree-checked');
						}
					}, this));
			}
	...
	$.jstree.plugins.dnd = function (options, parent) {
		this.bind = function () {
			...
			this.element
				.on('mousedown.jstree touchstart.jstree', '.treenode-cont .dnd', $.proxy(function (e) {
	...
	$(function () {
		...
		$(document)
			...
			.on('dnd_move.vakata.jstree', function (e, data) {
				...
				if (!data || !data.data || !data.data.jstree) { return; }
				// data.event.target = data.element.parentNode.parentNode;
				...
				if (ins && ins._data && ins._data.dnd) {
					...
					if ((data.event.target === ins.element[0] || data.event.target === ins.get_container_ul()[0]) && ins.get_container_ul().children().length === 0) {
						...
						ref = $(data.event.target).closest('.treenode-cont');
