	...
	$thumbindex=0;
		...
		$thumbindex++;
                if(sizeof($va_reps) > 1){
                        ...
                        $vn_padding_top = ((120 - $va_rep["info"]["thumbnail"]["HEIGHT"])/2) + 50;
                        //var_dump($va_reps);
                        print "<td>".$va_rep['tags']['thumbnail']."</td>";

			if(!($thumbindex % 6) || $thumbindex == sizeof($va_reps)) {
                                print "</tr>";
                        }

                }else{
                        # --- one rep - show medium rep
                        print "<td>" . $va_rep['tags']['medium']."</td></tr>";
                }
        }
?>
</tr></table></div>
        </div>
<table class="csi-content-table">
<?php
        foreach($va_placements as $vn_placement_id => $va_bundle_info){
                ...
                print "<tr><th>{$va_bundle_info['display']}</th><td>{$vs_display_value}</td></tr>";
        }
        print "</table>";
